document.addEventListener('DOMContentLoaded', () => {
  ...
  // ---- Pat Chatbot ----
  const patChatInput = document.getElementById('pat-chat-input');
  const sendPatChatBtn = document.getElementById('send-pat-chat-btn');
  const patChatWindow = document.getElementById('pat-chat-window');

  if (sendPatChatBtn && patChatInput && patChatWindow) {
    sendPatChatBtn.addEventListener('click', sendMessageToPat);
    patChatInput.addEventListener('keydown', (e) => {
  ...
  async function sendMessageToPat() {
    const message = patChatInput.value.trim();
    if (!message) return;

    addPatMessage(message, true);
    patChatInput.value = '';

    // Show loading or typing indicator if desired
    const loadingId = addPatMessage('Thinking...', false, true);

    try {
      const response = await fetch(`${NODE_API}/assistant`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: 'user_' + Date.now(), message }) // Simple user ID for now
      });
      ...
      if (data.error) {
        addPatMessage('Sorry, I encountered an error: ' + data.error);
      } else {
        addPatMessage(data.reply);
      }
    } catch (error) {
      const loadingMsg = document.getElementById(loadingId);
      if (loadingMsg) loadingMsg.remove();
      addPatMessage('Sorry, I cannot connect to the server right now.');
      console.error('Chat error:', error);
    }
  }

  function addPatMessage(text, isUser = false, isLoading = false) {
    const msgDiv = document.createElement('div');
    msgDiv.className = `chat-message ${isUser ? 'user' : 'assistant'} ${isLoading ? 'loading' : ''}`;
    msgDiv.textContent = text;
    if (isLoading) msgDiv.id = 'msg-' + Date.now();
    patChatWindow.appendChild(msgDiv);
    patChatWindow.scrollTop = patChatWindow.scrollHeight;
    return msgDiv.id;
  }
});
